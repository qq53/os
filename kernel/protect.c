...
/*
  -----------------------------------
  Create date: 2013-11-23 22:14
  Modified date: 2014-01-15 23:01
  Author: Sen1993
...
PUBLIC void init_prot()
{
	...
	init_descriptor(&gdt[INDEX_TSS], (u32)&tss,
			sizeof(tss) - 1, DA_386TSS);
	tss.iobase = sizeof(tss); /* 没有I/O许可位图 */

	/* 填充 GDT 中进程的 LDT_TASK 的描述符 */
	init_descriptor(&gdt[INDEX_LDT_TASK], (u32)ldts_task,
			LDT_SIZE * sizeof(DESCRIPTOR) - 1, DA_LDT);

	/* 填充 GDT 中进程的 LDT_USER 的描述符 */
	init_descriptor(&gdt[INDEX_LDT_USER], (u32)ldts_user,
			LDT_SIZE * sizeof(DESCRIPTOR) - 1, DA_LDT);
}
...
PUBLIC void exception_handler(u32 vec_no,u32 err_code,u32 eip,u32 cs,u32 flags)
{
	int i;
	int text_color = 0x74;
